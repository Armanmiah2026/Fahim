...
import renz.javacodez.vpn.utils.Utils;
import renz.javacodez.vpn.utils.Utils.*;
...
public abstract class OpenVPNClientBase extends AppCompatActivity implements InjectorService.InjectorListener, EventReceiver {
    ...
    public static InjectorService mInjector;
    private ServiceConnection mInjectorConnection = new ServiceConnection() {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        RootBeer root = new RootBeer(this);
        if(root.isRooted() || root.checkSuExists() || root.checkForMagiskBinary() || root.checkForBusyBoxBinary()) {
            showToast("Rooted device detected!");
            finish();
            return;
        }
    ...
    protected JSONObject getJSONObject()
    {
        ...
        try {
            if (file.exists()) {
                String jsonEncrypted = readStream(new FileInputStream(file));
                return new JSONObject(Utils.Parser.decryptString(jsonEncrypted));
            } else {
                String s = ConfigUtil.CONFIG;
                String s2 = Parser.decryptString(s);
    ...
    protected void createAppShortcut(String shortcut_name) {
        createShortcut(shortcut_name, new Intent(this, OpenVPNClient.class).setAction("android.intent.action.RUN"), R.drawable.icon);
    }
