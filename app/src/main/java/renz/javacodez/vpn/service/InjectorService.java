...
import renz.javacodez.vpn.thread.*;
...
public class InjectorService extends Service implements Runnable, Handler.Callback
{
    ...
    {
        NotificationChannel channel = new NotificationChannel(NOTIFICATION_CHANNEL_ID, "Injector Notification",NotificationManager.IMPORTANCE_DEFAULT);
        channel.setShowBadge(true);
        channel.setDescription("Notification showing if the Injector Service is still running");
    ...
    private boolean connectSocket() throws Exception
    {
        try {
            ...
            while (true) {
                ...
                log("Selected Server",config.getServerSelectedName());
                if (mTunnelType != ConfigUtil.MODE_OVPN_DIRECT) {
                    log("Selected Network", config.getNetworkSelectedName());
                }

                //String[] tunnel_types = {"OVPN Direct", "OVPN Direct + Payload", "OVPN HTTP Proxy + Payload", "SSL Direct", "SSL Direct + Payload", "SSL HTTP Proxy + Payload", "UDP Direct", "UDP SSL Direct", "UDP SSL > TLS Proxy"};
                ...
                if (mTunnelType == ConfigUtil.MODE_OVPN_DIRECT) {
                ...
                } else if (mTunnelType == ConfigUtil.MODE_OVPN_DIRECT_WITH_PAYLOAD) {
                    ...
                    send200Status(client.getOutputStream());
                } else if (mTunnelType == ConfigUtil.MODE_OVPN_HTTP_PROXY) {
                    ...
                    //send200Status(client.getOutputStream());
                } else if (mTunnelType == ConfigUtil.MODE_SSL_DIRECT || mTunnelType == ConfigUtil.MODE_UDP_SSL_DIRECT) {
                    ...
                    send200Status(client.getOutputStream());
                } else if (mTunnelType == ConfigUtil.MODE_SSL_HTTP_PROXY || mTunnelType == ConfigUtil.MODE_UDP_SSL_PROXY) {
    ...
    private void retryOnFailure() throws Exception {
        ...
        while ((len = readLineRN(in, buffer)) != 0) {
            httpReponseAll += "\n";
            try {
        ...
        if (len < 0 || len > 999) {
            ...
            String stringBuffer = new StringBuffer().append(new StringBuffer().append("HTTP/1.1 200 OK").append("\r\n").toString()).append("\r\n").toString();
    ...
    public int readLineRN(InputStream is, byte[] buffer) throws IOException
    {
        int pos = 0;
        ...
        if (str != null) {
            try {
                if (!str.equals("")) {
                    ...
                    str2 = d(config.getPayload().replace("[rlb]", config.getSSHHost()).replace("JX", config.getSSHHost()).replace("[real_raw]", str).replace("[raw]", charSequence).replace("[method]", split[0]).replace("[host_port]", split[1]).replace("[host]", host).replace("[port]", port).replace("[protocol]", split[2]).replace("[cr]", "\r").replace("[lf]", "\n").replace("[crlf]", "\r\n").replace("[lfcr]", "\n\r").replace("\\r", "\r").replace("\\n", "\n"));
    ...
    public void run()
    {
        ...
        try {
            log(String.format("Listening to localport %s", Integer.toString(8989)));
            log("Listening for incoming connection");

            ss = new ServerSocket(8989);

            if (mTunnelType == ConfigUtil.MODE_SSL_DIRECT || mTunnelType == ConfigUtil.MODE_SSL_DIRECT_WITH_PAYLOAD || mTunnelType == ConfigUtil.MODE_SSL_HTTP_PROXY || mTunnelType == ConfigUtil.MODE_UDP_SSL_DIRECT || mTunnelType == ConfigUtil.MODE_UDP_SSL_PROXY) {
