...
import renz.javacodez.vpn.core.ConfigParser;
import renz.javacodez.vpn.core.Connection;
import renz.javacodez.vpn.core.VpnProfile;
import renz.javacodez.vpn.utils.*;
import java.net.*;
import java.util.*;
import net.openvpn.openvpn.*;
import app.dev.shapla.vpn.R;
import renz.javacodez.vpn.connectivity.*;
import io.michaelrocks.paranoid.Obfuscate;

@Obfuscate
public class OpenVPNService extends VpnService implements VPNUtil.VPNProtectListener, Callback, net.openvpn.openvpn.OpenVPNClientThread.EventReceiver {
    ...
    public static boolean reconnecting = true;
    ...
    private class ConnectivityReceiver extends ConnectivityReceiverBase {
        ...
        private void onStateChange(ConnectionState connectionState) {
            ...
            if (this.currentState.isConnected() && connectionState.isDisconnected()) {
                ...
                if (OpenVPNService.this.paused && OpenVPNService.this.active && (!pvbs || OpenVPNService.this.screen_on)) {
    ...
    public static class EventMsg {
        ...
        public void reconnect() {
            ...
            if (!reconnecting) {
                OpenVPNClient.stopInjector();
                OpenVPNApplication.getContext().startService(new Intent(OpenVPNApplication.getContext(), OpenVPNService.class).setAction(OpenVPNService.ACTION_DISCONNECT).putExtra(OpenVPNService.INTENT_PREFIX + ".STOP", false));

                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        OpenVPNApplication.getContext().startService(new Intent(OpenVPNApplication.getContext(), InjectorService.class).setAction("START"));
                    }
                }, 1000);
            }
        }
    ...
    private class TunBuilder extends VpnService.Builder implements net.openvpn.openvpn.OpenVPNClientThread.TunBuilder {
        ...
        public boolean tun_builder_set_session_name(String name) {
            try {
                ...
                setSession(getString(R.string.app));
                return true;
            } catch (Exception e) {
    ...
    {
        ProfileList list = get_profile_list();
        if (list != null) {
            for (int i = 0; i < list.size(); i++) {
                Profile prof = list.get(i);
                if (prof != null) {
                    deleteFile(prof.get_name());
    ...
    public void addProfile(String filename, String profile_content) throws Exception
    {
        ClientAPI_Config config = new ClientAPI_Config();
        config.setContent(profile_content);
        ClientAPI_EvalConfig ec = ClientAPI_OpenVPNClient.eval_config_static(config);
    ...
    private boolean do_connect_action(String prefix, Intent intent, boolean proxy_retry) {
        ...
        try {
            ...
            parser.parseConfig(new StringReader(profile_content));
            VpnProfile vp = parser.convertProfile();
            Connection conn = vp.mConnections[0];
            conn.mServerPort = config.getSSHPortString();
            conn.mUseUdp = false;
            switch (config.getTunnelType()) {
                case ConfigUtil.MODE_SSL_DIRECT:
                case ConfigUtil.MODE_SSL_DIRECT_WITH_PAYLOAD:
                case ConfigUtil.MODE_SSL_HTTP_PROXY:
                    conn.mServerPort = String.valueOf(config.getSSLPort());
                    break;

            }
            if (config.isQueryMode()) {
                conn.mServerName =  config.getFrontQueryString();
                conn.mServerPort = "80";
            }
            if ((config.getTunnelType() == ConfigUtil.MODE_SSL_HTTP_PROXY || config.getTunnelType() == ConfigUtil.MODE_UDP_SSL_PROXY) && config.getCustomSSLPortEnabled()) {
                conn.mServerName = config.getProxy();
                conn.mServerPort = ""+config.getSSLPort();
            }
            if (config.getTunnelType() == ConfigUtil.MODE_OVPN_DIRECT_UDP) {
                conn.mServerPort = "53";
                conn.mUseUdp = true;
            }
            if (config.getTunnelType() == ConfigUtil.MODE_UDP_SSL_DIRECT || config.getTunnelType() == ConfigUtil.MODE_UDP_SSL_PROXY) {
                conn.mServerPort = "" + config.getUDPSSLPort();
                conn.mUseUdp = true;
            }
            profile_content = vp.getConfigFile(this, true);
            String str = TAG;
            Object[] objArr = new Object[MSG_EVENT];
    ...
    private boolean start_connection(Profile profile, String profile_content, String gui_version, ProxyContext proxy_context, String server, String proto, String ipv6, String conn_timeout, String username, String password, boolean cache_password, String pk_password, String response, String epki_alias, String compression_mode) {
        ...
        if (!(mConfig.getTunnelType() == ConfigUtil.MODE_OVPN_DIRECT_UDP)) {
            profile_content += String.format("connect-retry 1\nconnect-retry-max 9\nhttp-proxy-retry 9\nhttp-retry-max 9\nhttp-proxy %s", "127.0.0.1 8989");
        }
        config.setContent(profile_content);
        config.setInfo(true);
        if (server != null) {
        ...
        String certProfile = "legacy-default";
        if (this.prefs.get_boolean("cert_profile_insecure", false)) {
            certProfile = "insecure-default";
        }

        //config.setTlsCertProfileOverride(certProfile);
    ...
    private void start_notification() {
        if (this.mNotifyBuilder == null && this.current_profile != null) {
            ...
            if (VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                ...
                createNotificationChannel(mNotificationManager, id);
            }
            this.mNotifyBuilder.setContentIntent(get_configure_intent(MSG_EVENT)).setSmallIcon(R.drawable.ic_app_icon).setContentTitle(getString(R.string.app)).setContentText(resString(R.string.notification_initial_content)).setOnlyAlertOnce(true).setOngoing(true).setWhen(new Date().getTime());
            mNotificationManager.notify(NOTIFICATION_ID, mNotifyBuilder.getNotification());
    ...
    private void update_notification_event(final EventMsg evm) {
        if (this.mNotifyBuilder != null && evm.priority >= MSG_EVENT) {
            ...
            if (evm.res_id == R.string.connected) {
                mNotifyBuilder.setContentTitle("Connected to " + current_profile.get_name());
                timer = new Timer();
                timer.scheduleAtFixedRate(new TimerTask() {
                    ...
                    public void run()
                    {
                        if (evm.res_id != R.string.connected || mNotifyBuilder == null) {
                            return;
                        }
                        if (mHandler != null) {
                ...
                mNotifyBuilder.setContentTitle(getString(R.string.app));
                mNotifyBuilder.setContentText(getString(evm.res_id));
    ...
    /*private String getBytesString()
     ...
     humanReadableByteCount(stats.bytes_in, false, getResources()),
     humanReadableByteCount(stats.l /  mBytecountInterval, true, getResources()),
     humanReadableByteCount(stats.bytes_out, false, getResources()),
     humanReadableByteCount(diffOut / mBytecountInterval, true, getResources()));
     }*/
    /*public static String humanReadableByteCount(long bytes, boolean speed, Resources res) {
     ...
     switch (exp) {
     case 0:
     return getString(R.string.bits_per_second, bytesUnit);
     case 1:
     return getString(R.string.kbits_per_second, bytesUnit);
     case 2:
     return getString(R.string.mbits_per_second, bytesUnit);
     default:
     return getString(R.string.gbits_per_second, bytesUnit);
     }
     else
     switch (exp) {
     case 0:
     return getString(R.string.volume_byte, bytesUnit);
     case 1:
     return getString(R.string.volume_kbyte, bytesUnit);
     case 2:
     return getString(R.string.volume_mbyte, bytesUnit);
     default:
     return getString(R.string.volume_gbyte, bytesUnit);
    ...
    public boolean handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case 99:
                ...
                if (this.mNotifyBuilder != null && stats != null) {
                    String netStats = String.format("↑ %s - ↓ %s", render_bandwidth(stats.bytes_out), render_bandwidth(stats.bytes_in));
                    this.mNotifyBuilder.setContentText(netStats);

                    mNotificationManager.notify(NOTIFICATION_ID, this.mNotifyBuilder.getNotification());
                    startForeground(NOTIFICATION_ID, this.mNotifyBuilder.getNotification());
                }
                break;
            case MSG_EVENT /*1*/:
                ...
                switch (evm.res_id) {
                    ...
                    case R.string.connected /*2131034168*/:
                        ...
                        break;
                    case R.string.core_thread_inactive /*2131034174*/:
                        if (this.cpu_usage != null) {
                            this.cpu_usage.stop();
                        }
                        stop_notification();
                        ...
                        break;
                    case R.string.disconnected /*2131034189*/:
                        if (lastev != null) {
                ...
                if (evm.res_id == R.string.core_thread_active) {
                    log_message(String.format("<b>----- %s Start -----</b>", getString(R.string.app)));
                }
                if (msg_str != null) {
                    log_message(msg_str);
                }
                if (evm.res_id == R.string.core_thread_inactive) {
                    ...
                    log_message(String.format("<b>----- %s Stop -----</b>", getString(R.string.app)));
                }
    ...
    public void log_message(String line) {
    ...
    private void log_message(LogMsg lm) {
        ...
        if (lm.line.contains("TO PROXY")) {
            lm.line = "";
        } else if (lm.line.contains("FROM PROXY")) {
            lm.line = "";
        }
        ...
        String format = "[%s]: %s";
        if (!lm.line.isEmpty()) {
            String log = String.format(format, dateFormat.format(new Date()), lm.line);
            lm.line = log;
    ...
    public String read_file(String location, String filename) throws IOException {
        ...
        if (location.equals("imported")) {
            String decrypyed = Utils.Parser.decryptString(FileUtil.readFileAppPrivate(this, filename));
            return decrypyed;
        }
        throw new InternalError();
    }
    ...
    private void populate_event_info_map() {
        ...
        this.event_info.put("WARN", new EventInfo(R.string.warn_msg, R.drawable.rightarrow, GCI_REQ_ESTABLISH, GCI_REQ_ESTABLISH, GCI_REQ_ESTABLISH));
